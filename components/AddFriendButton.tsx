...
const AddFriendButton: FC = () => {
  ...
  const addFriend: (data: Email) => Promise<void> = async (data: Email) => {
    try {
      if (!validateEmail(data)) {
        toast.error("Enter valid email");
        return;
      }
      await add(JSON.parse(JSON.stringify(data)) as Email);
      ...
      setError(`email`, { message: `Something went wrong ${e}` });
    }
  };

  return (
    <form
      onSubmit={handleSubmit((data: Email) => {
        addFriend(data);
      })}
      className="max-w-sm"
    >
